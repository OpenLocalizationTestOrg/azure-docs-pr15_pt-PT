<properties
   pageTitle="Vistas de sistema do armazém de dados SQL | Microsoft Azure"
   description="Ligações ao sistema de vistas conteúdo para armazém de dados SQL."
...
## <a name="sql-data-warehouse-catalog-views"></a>Vistas de catálogo armazém de dados SQL
...
## <a name="sql-data-warehouse-dynamic-management-views-dmvs"></a>Vistas de gestão de dinâmica armazém de dados SQL (DMVs)
...
## <a name="sql-server-dmvs-applicable-to-sql-data-warehouse"></a>SQL Server DMVs aplicável ao armazém de dados SQL

As seguintes DMVs são aplicáveis a armazém de dados SQL, mas tem de ser executados por ligar à base de dados **principal** .
...
## <a name="sql-server-dmvs-available-in-sql-data-warehouse"></a>SQL Server DMVs disponíveis no armazém de dados SQL

Armazém de dados SQL expõe muitas das vistas dinâmicas de gestão do SQL Server (DMVs). Estas vistas, quando consultado no armazém de dados SQL, são reportar o estado das bases de dados do SQL em execução nas distribuições.

Uma vez armazém de dados SQL está incorporada no tecnologia MPP da Microsoft, armazém de dados SQL e paralelas dados armazém (PDW do sistema de análise da plataforma) utilizam as mesmas vistas do sistema.

Este é o motivo pelo qual cada um dos seguintes de DMV tem uma coluna específica denominada pdw_node_id. Este é o identificador para o nó cluster. No PDW a cluster nó é um conceito mais forte para a arquitetura. No armazém de dados do SQL, a arquitetura baseia-se mais fortemente nas distribuições.

>[AZURE.NOTE] Para utilizar estes vista, Inserir 'pdw_nodes_' o nome, conforme apresentado na seguinte tabela.


| Nome das DMV no armazém de dados SQL | Ligação para o tópico do SQL Server Transact-SQL no MSDN |
| :----------------------------- | :-------------------------------------------- |
...
| sys.dm_pdw_nodes_os_dispatchers | Documentação Transact-SQL não está disponível. |
...
| sys.dm_pdw_nodes_os_memory_node_access_stats | Documentação Transact-SQL não está disponível. |
...
| sys.dm_pdw_nodes_os_spinlock_stats | Documentação Transact-SQL não está disponível. |
...
## <a name="sql-server-2016-polybase-dmvs-available-in-sql-data-warehouse"></a>SQL Server 2016 PolyBase DMVs disponíveis no armazém de dados SQL
...
## <a name="sql-server-informationschema-views"></a>Vistas de vista de INFORMATION_SCHEMA do SQL Server
