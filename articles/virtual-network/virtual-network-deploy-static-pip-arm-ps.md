<properties 
   pageTitle="Implementar uma VM com um endereço IP público estático através do PowerShell no Gestor de recursos | Microsoft Azure"
   description="Saiba como implementar VMs com um endereço IP público estático através do PowerShell no Gestor de recursos"
...
# <a name="deploy-a-vm-with-a-static-public-ip-using-powershell"></a>Implementar uma VM com um endereço IP público estático através do PowerShell
...
Pode transferir o script PowerShell completo utilizado [aqui](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/03-Static-public-IP/virtual-network-deploy-static-pip-arm-ps.ps1). Siga os passos abaixo para alterar o script para trabalhar no seu ambiente.
...
4. Crie a interface de rede (NIC) para a VM na sub-rede criada acima, com o endereço IP público. Repare que o cmdlet primeiro a obter o VNet a partir do Azure, isto é necessário uma vez que um **Conjunto AzureRmVirtualNetwork** tiver sido executado para alterar o VNet existente.
...
1. Obter as credenciais para a conta de administrador local VM.

        $cred = Get-Credential -Message "Type the name and password for the local administrator account."

2. Crie um objeto de configuração de VM.

        $vmConfig = Set-AzureRmVMOperatingSystem -VM $vmConfig -Windows -ComputerName $vmName `
            -Credential $cred -ProvisionVMAgent -EnableAutoUpdate

3. Defina a imagem do sistema operativo para a VM.

        $vmConfig = Set-AzureRmVMSourceImage -VM $vmConfig -PublisherName $publisher `
            -Offer $offer -Skus $sku -Version $version

4. Configure o disco SO.

        $osVhdUri = $stdStorageAccount.PrimaryEndpoints.Blob.ToString() + "vhds/" + $osDiskName + ".vhd"
        $vmConfig = Set-AzureRmVMOSDisk -VM $vmConfig -Name $osDiskName -VhdUri $osVhdUri -CreateOption fromImage

5. Adicione a imagem para a VM.
