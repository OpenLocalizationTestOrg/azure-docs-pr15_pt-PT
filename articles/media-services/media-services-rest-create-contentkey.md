...
#<a name="create-contentkeys-with-rest"></a>Criar ContentKeys com restantes
...
Dos serviços de multimédia permite-lhe criar novas e entregar activos encriptados. Um **ContentKey** fornece acesso seguro a sua s **ativo**. 

Quando cria um novo activo (por exemplo, antes de [carregar ficheiros](media-services-rest-upload-files.md)), pode especificar as seguintes opções de encriptação: **StorageEncrypted**, **CommonEncryptionProtected**ou **EnvelopeEncryptionProtected**. 

Quando medida que faz a elementos para os seus clientes, pode [configurar para recursos ser dinamicamente encriptados](media-services-rest-configure-asset-delivery-policy.md) com um da duas encriptação seguintes: **DynamicEnvelopeEncryption** ou **DynamicCommonEncryption**.
...
1. Aleatoriamente gera uma chave AES de 16 byte (para encriptação de envelope e comum) ou uma chave AES 32 byte (para encriptação de armazenamento). 
...
3.  Encripte a sua chave de conteúdo com a chave do certificado x. 509 pública. 
    ...
    Serviços de multimédia .NET SDK utiliza RSA com OAEP quando efetuar a encriptação.  Pode ver um exemplo da [função EncryptSymmetricKeyData](https://github.com/Azure/azure-sdk-for-media-services/blob/dev/src/net/Client/Common/Common.FileEncryption/EncryptionUtils.cs).
4.  Crie um valor de soma (com base no algoritmo de soma verificação chave AES de PlayReady) calculado utilizando o identificador de chave e chave de conteúdo. Para obter mais informações, consulte a secção "PlayReady AES chave algoritmo de soma de verificação" do documento PlayReady cabeçalho objeto encontra [aqui](http://www.microsoft.com/playready/documents/).

    O exemplo .NET seguinte calcula a soma de verificação utilizar a peça GUID do identificador chave e a chave de limpar conteúdo.
...
6. Associe a entidade de **ContentKey** a sua entidade de **elementos** através da operação de $links.

Tenha em atenção que exemplos que geram uma tecla AES, encriptar a chave e calcular a soma de verificação foram omitidos a partir deste tópico. Apenas os exemplos que mostram como interagir com os serviços de multimédia são fornecidos.
...
Criar um dos valores que tem de definir quando o conteúdo chave é o tipo. Escolha uma das seguintes valores.
