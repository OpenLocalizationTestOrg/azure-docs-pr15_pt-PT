<properties 
    pageTitle="Implementar a activação pós-falha transmissão cenário | Microsoft Azure" 
    description="Este tópico mostra como implementar o activação pós-falha transmissão cenário." 
...
#<a name="implementing-failover-streaming-scenario"></a>Implementar a activação pós-falha transmissão cenário

Este tutorial demonstra como copiar conteúdos (blobs) a partir de um ativo para outra para processar redundância de transmissão da pedido. Este cenário é útil para clientes que pretende configurar os respetivos CDN para activação pós-falha entre dois centros de dados no caso de uma falha das nossos centros de dados.
Este tutorial utiliza SDK de serviços de multimédia do Microsoft Azure, Microsoft Azure multimédia serviços REST API e Azure armazenamento SDK para demonstrar as seguintes tarefas.

1. Configure uma conta dos serviços de multimédia no "Centro de dados A".
1. Carregar um ficheiro de mezzanine para um recurso de origem.
1. Codificar activo em ficheiros de vários bits taxa MP4. 
1. Crie um localizador de SA só de leitura para o ativo de origem ter acesso de leitura para o contentor na conta de armazenamento que está associada o elementos de origem.
1. Obter o nome do contentor do elemento origem a partir do localizador de SA de só de leitura, criado no passo anterior. Precisamos estas informações para copiar blobs entre contas de armazenamento (é explicadas posteriormente o tópico.)
1. Crie um localizador de origem para o ativo que foi criado pela codificação tarefa. 

Em seguida, para processar a activação pós-falha:

1. Configure uma conta dos serviços de multimédia no "B do Centro de dados".
1. Crie um activo vazia de destino na conta dos serviços de multimédia de destino.
1. Crie um localizador de SA escrita para o destino vazia de elementos ter acesso de escrita para o contentor na conta de armazenamento que está associada o elementos de destino de destino.
1. Utilizar o Azure armazenamento SDK para copiar blobs (ficheiros de recurso) entre a conta de armazenamento de origem no "Centro de dados A" e a conta de armazenamento de destino no "B do Centro de dados" (nestas contas de armazenamento estão associadas a recursos de interesse.)
1. Associe blobs (ficheiros de recurso) que foram copiados para o contentor de Blobs do destino com o activo de destino. 
1. Crie um localizador de origem para o item "B do Centro de dados" e especifique o localizador de Id do que foi gerado pelo activo numa "Centro de dados A". 
1. Isto dá-lhe os URLs transmissão onde os caminhos relativos dos URLs são os mesmos (apenas a base URLs são diferentes). 
 
Em seguida, para processar quaisquer interrupções, pode criar uma CDN na parte superior deste Locator de origem. 

As seguintes considerações aplicam-se:

- A versão atual do SDK de serviços de multimédia não suporta a criação de um locator com um id de locator especificado. Para realizar esta tarefa, utilizamos REST API serviços de multimédia.
- A versão atual do SDK de serviços de multimédia não suporta através de programação gerar IAssetFile informações que pretende associar um ativo ficheiros ativo. Para realizar esta tarefa, utilizamos CreateFileInfos multimédia serviços REST API. 
- Elementos de armazenamento encriptado (AssetCreationOptions.StorageEncrypted) não são suportados para a replicação (uma vez que a chave de encriptação irão ser diferente em ambas as contas dos serviços de multimédia). 
- Se pretender tirar partido da embalagem dinâmico, primeiro terá de obter pelo menos uma pedido transmissão reservadas unidades. Para mais informações, consulte o artigo [Dinamicamente embalagem elementos](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Considere utilizar a [Ferramenta de replicação](http://replicator.codeplex.com/) de dos serviços de multimédia como alternativa a um cenário de transmissão de activação pós-falha de execução manualmente. Esta ferramenta permite-lhe a ser replicadas elementos em duas contas dos serviços de multimédia.

##<a name="prerequisites"></a>Pré-requisitos
 
- Duas contas dos serviços de multimédia numa subscrição Azure nova ou existente. Veja [como criar uma conta de serviços de multimédia](media-services-portal-create-account.md).
- Sistemas operativos: Windows 7, Windows 2008 R2 ou Windows 8.
...
1. Crie a pasta SupportFiles no mesmo nível que o ficheiro de projeto HandleRedundancyForOnDemandStreaming.csproj. Na pasta SupportFiles crie as pastas OutputFiles e MP4Files. Copie um ficheiro. mp4 para a MP4Files pasta (neste exemplo que é utilizado o ficheiro BigBuckBunny.mp4). 
...
1. Adicionar outras referências que são necessárias para este projeto: System.Configuration, System.Runtime.Serialization e System. Web.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Adicione código que processa redundância de transmissão da pedido.



1. Adicione os seguintes campos de nível de classe para a classe de programa.
...
1. Substitua a definição de método predefinido principais pelo seguintes:
...
1. Definições de métodos que são chamadas a partir principais são definidas abaixo.
...
Agora pode usar um Gestor de tráfego para encaminhar os pedidos entre os centros de dados de duas e assim activação pós-falha no caso de qualquer corrente.
